    ...
    try {
      ...
      while ((fromServer = in.readObject()) != null) {
        ...
        if (clientProtocol.isAutomobile(fromServer))
          toServer = clientProtocol.configureAuto(fromServer);

        System.out.print(">> Response to server: ");

        if (toServer == null || !clientProtocol.isAutomobile(toServer)) toServer = stdIn.readLine();
        ...
        if (DEBUG) System.out.println("Sending \"" + toServer + "\" to server ... ");
        ...
        if (toServer.equals("0")) {
          break;
        } else if (clientProtocol.isAutomobile(toServer)) {
          toServer = null;
        }
      }
